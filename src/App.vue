...
function gettotal(){
  let total = data.subTotal + (data.subTotal * data.tax / 100);
  data.total = total;
   return total;
}
        ...
        <div class="mt-20">
            <table class="table-auto w-full">
                <tr class="bg-gray-800 text-left text-white">
                    ...
                    <th class="p-2 w-[50px] text-right pr-5"></th>
                </tr>
                <tr v-for="(item , index) in data.items" :key="index">
                    ...
                    <td class="py-1 pr-5 text-right text-gray-800">
                        <p @click="removeItem(index)" style="background-color:red;color:white;font-weight:bold;text-align:center;cursor: pointer;" class="bg-red"> &#x2715;</p>
                    </td>
                </tr>
            </table>
            <button @click="addMore()" class="mt-5 bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">
                Add More
            </button>
        </div>
        <div class="mt-[200px]">
